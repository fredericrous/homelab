...
#
# Bootstrap flow:
# 1. platform-foundation deploys (Rook-Ceph, Vault, ESO, Istio)
# 2. Vault initializes and unseals
# 3. External Secrets Operator becomes ready
# 4. security, serverless, data-cache layers deploy in parallel
# 5. VaultStaticSecret creates cert-manager-env-config from Vault
# 6. ml-serving and data-storage layers deploy
# 7. apps layer deploys with all dependencies ready
